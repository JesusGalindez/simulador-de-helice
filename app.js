...
const App = {
  ...
  bindEvents() {
    ...
    // Google Colab CFD Event Listeners (Method B)
    const btnLoadCFD = document.getElementById('btn-load-cfd');
    const btnTogglePhysics = document.getElementById('btn-toggle-physics');
    const gdriveInput = document.getElementById('gdrive-url');
    const cfdStatus = document.getElementById('cfd-status');

    btnLoadCFD.addEventListener('click', async () => {
      const url = gdriveInput.value.trim();
      if (!url) {
        alert('Por favor, pega un enlace válido de Google Drive o un archivo JSON.');
        return;
      }

      cfdStatus.innerText = 'Conectando a Google Cloud...';
      cfdStatus.style.color = 'var(--accent-toroidal)';
      btnLoadCFD.innerText = 'Cargando...';
      btnLoadCFD.disabled = true;
      ...
      try {
        // Fetch through our secure Vercel Serverless CORS bypass proxy
        const response = await fetch(`/api/fetch-cfd?url=${encodeURIComponent(fetchUrl)}`);
        if (!response.ok) throw new Error('Error al descargar el archivo.');
        
        const data = await response.json();
        
        if (!data.particles || !Array.isArray(data.particles)) {
          throw new Error('El formato del archivo no contiene la estructura CFD toroidal.');
        }

        // Successfully loaded!
        this.state.cfdData = data;
        btnTogglePhysics.disabled = false;
        btnTogglePhysics.style.borderColor = 'var(--accent-efficiency)';
        btnTogglePhysics.style.color = 'var(--color-text-primary)';
        
        cfdStatus.innerText = 'CFD Cargado con Éxito';
        cfdStatus.style.color = 'var(--accent-efficiency)';
        btnLoadCFD.innerText = 'CFD Listo';
        
        // Auto-switch to CFD view to wow the user
        this.togglePhysicsMode(true);
        
      } catch (err) {
        console.error(err);
        cfdStatus.innerText = 'Error de conexión / CORS';
        cfdStatus.style.color = 'var(--accent-standard)';
        btnLoadCFD.innerText = 'Reintentar';
        btnLoadCFD.disabled = false;
        alert(`No se pudo conectar a los datos de Google Drive. Asegúrate de que el archivo es público ("Cualquier persona con el enlace"). Detalle: ${err.message}`);
      }
    });

    btnTogglePhysics.addEventListener('click', () => {
      this.togglePhysicsMode(!this.state.useCFD);
    });
  },

  togglePhysicsMode(enableCFD) {
    this.state.useCFD = enableCFD;
    const btnTogglePhysics = document.getElementById('btn-toggle-physics');
    const cfdStatus = document.getElementById('cfd-status');

    if (this.state.useCFD) {
      btnTogglePhysics.innerText = 'Ver Procedimental';
      btnTogglePhysics.classList.add('active');
      cfdStatus.innerText = 'Modo: Google Cloud CFD';
      cfdStatus.style.color = 'var(--accent-efficiency)';
      
      // Override UI labels to show GPU data
      const meta = this.state.cfdData.metadata;
      if (meta) {
        document.getElementById('detail-regime').innerText = 'CFD Completo (GPU)';
        document.getElementById('detail-regime').style.color = 'var(--accent-toroidal)';
        document.getElementById('rpm-slider').value = meta.rpm;
        this.state.rpm = meta.rpm;
        document.getElementById('rpm-val').innerText = `${meta.rpm} RPM (Colab)`;
        document.getElementById('pitch-slider').value = meta.pitch;
        this.state.pitch = meta.pitch;
        document.getElementById('pitch-val').innerText = `${meta.pitch}° (Colab)`;
        document.getElementById('inflow-slider').value = meta.inflow;
        this.state.inflow = meta.inflow;
        document.getElementById('inflow-val').innerText = `${meta.inflow} m/s (Colab)`;
        this.updatePropellerGeometry();
        this.updatePhysics();
      }
    } else {
      btnTogglePhysics.innerText = 'Ver CFD Real';
      btnTogglePhysics.classList.remove('active');
      cfdStatus.innerText = 'Modo: Procedimental';
      cfdStatus.style.color = 'var(--color-text-secondary)';
      this.updatePhysics();
    }
  },
